define(['jquery', "components", "bootstrap", "manageCommon"], function(jquery, components, bootstrap, manageCommon) {
    ...
    listUrl = apiUrl + "/admin/article/articleAdmin/getArticleByPage";
    require(['./table']);
    ...
    function delMessage() {
        $("#table-list-content").on("click", ".del-btn", function() {
            var url = apiUrl + "/admin/article/articleAdmin/delArticleById?articleId=" + $(this).attr("god");
            $("#del-modal").modal('show');
            $("#del-modal #name").html('确定是否删除：' + ' "'+ $($(this).parent("td").siblings()[1]).text().trim() + '" 文章');
